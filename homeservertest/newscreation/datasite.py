...
def get_source(content_link):
    source = content_link.split('/')[2]
    return source

def convert_to_path(url, cat):
    str_url = str(url)
...
def save_image(url, image_path):
    ...
    path_image = '/home/michael/code/homeserversolution/homeservertest/media/' + image_path
    with open(path_image, 'wb') as f:
        f.write(image)


def make_format_date(date_text):
